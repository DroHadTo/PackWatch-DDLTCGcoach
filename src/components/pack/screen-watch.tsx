  ...
  async function sample(deep: boolean) {
    ...
    try {
      ...
      if (TD) {
        ...
        const hits = await detectorRef.current.detect(bmp);
        text = hits.map((h) => h.rawValue).join(" ").replace(/\s+/g, " ").trim();
        bmp.close();
      } else if (!deep) {
        setErr("This browser cannot read shared-tab text locally. Use Chrome for live reading, or call the board here.");
      }
    } catch {
    ...
    <div className="flex flex-col gap-3">
      ...
      {!compact && (
        <p className="text-xs leading-relaxed text-faint">
          Safety: Pack Watch never injects into DDL, never clicks, never reads cookies, and never sees a window you
          did not share. It does not require a Chrome extension.
        </p>
      )}
    </div>
  );
